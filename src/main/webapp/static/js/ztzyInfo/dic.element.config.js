...
$(function(){
	//初始化表格,动态从服务器加载数据  
	$table=$("#dicElementList").bootstrapTable({
		method : "GET", //使用get请求到服务器获取数据  
		url : path + "/ompDicElement/getPageList", //获取数据的地址  
        ...
        queryParams : function queryParams(params) {  //设置查询参数  
          var param = {
	            ...
	            elemName:encodeURI($("#elemName").val())
          };    
          return param;                   
        },
		...
		columns : [
		   ...
		   {field : 'typeCode',title : '分类类型',formatter:showAllName},
		   {field : 'elemCode',title : '分类代码',formatter:showAllName},
		   {field : 'elemName',title : '分类名称',formatter:showAllName},
		   {field : 'elemDesc',title :'分类描述',formatter:showAllName},
		   {field : 'sort',title : '排序',formatter:showAllName},
...
$(function (){
	$.ajax({
		url: path+'/ompDicElement/findAll',
...
function view(id){
	...
	$("#editModal").modal({  
	    remote: path + "/ompDicElement/gotoView?id=" + id
	}); 
	//设置弹出框的最大高度
	setModalBodyHeight();
}
...
function add(option){
	...
	$("#editModal").modal({  
	    remote: path + "/ompDicElement/gotoAdd?action="+ option
	});
	//设置弹出框的最大高度
	setModalBodyHeight();
}
...
function update(id){
	...
	$("#editModal").modal({  
	    remote: path + "/ompDicElement/gotoUpdate?id="+ id
	});
	//设置弹出框的最大高度
	setModalBodyHeight();
}
...
function deleteCurrentRow(id){
	layer.confirm('删除后无法恢复您确定要删除?', function(index){
		$.ajax({
			type : 'get',
			url : path + "/ompDicElement/delete?id=" + id,
			success : function(data) {
				if(data=="success"){
