...
$(function(){
	//初始化表格,动态从服务器加载数据  
	$table=$("#ztzyList").bootstrapTable({
		method : "GET", //使用get请求到服务器获取数据  
		url : path + "/pipZtzyInfo/getPageList", //获取数据的地址  
        ...
        queryParams : function queryParams(params) {  //设置查询参数  
          var param = {
	            ...
	            ztbh:encodeURI($("#ztbh").val()),
	            ztmc:encodeURI($("#ztmc").val())
          };    
          return param;                   
        },
		...
		columns : [
		   ...
		   {field : 'ztbh',title : '专题编号',width:'70px',formatter:showAllName},
		   {field : 'ztmc',title : '专题名称',width:'150px',formatter:showAllName},
		   {field : 'fl',title : '分类',width:'100px',formatter:showAllName},
		   {field : 'bq',title :'球的标签',width:'100px',formatter:showAllName},
		   {field : 'threedZxfs',title : '3D展现方式',width:'100px',formatter:showAllName},
		   {field : 'ztUrl',title : '专题Url',width:'300px',formatter:showAllName},
		   {field : 'sltUrl',title : '缩略图Url',formatter:showAllName},
		   {field : 'sort',title : '排序',width:'50px',formatter:showAllName},
		   {field : 'isValid',title : '数据状态',width:'70px',formatter:isEnableFormatter},
...
function operateFormatter(value, row, index){
	var html ='<button title="查询详情" type="button" class="btn btn-primary" onclick="view(\''+row.id+'\')">详情</button>' 
...
function view(id){
	...
	$("#editModal").modal({  
	    remote: path + "/pipZtzyInfo/gotoView?id=" + id
	}); 
	//设置弹出框的最大高度
	setModalBodyHeight();
}
...
function add(option){
	...
	$("#editModal").modal({  
	    remote: path + "/pipZtzyInfo/gotoAdd?action="+ option
	});
	//设置弹出框的最大高度
	setModalBodyHeight();
}
...
function update(id){
	...
	$("#editModal").modal({  
	    remote: path + "/pipZtzyInfo/gotoUpdate?id="+ id
	});
	//设置弹出框的最大高度
	setModalBodyHeight();
}
...
function deleteCurrentRow(id){
	layer.confirm('删除后无法恢复您确定要删除?', function(index){
		$.ajax({
			type : 'get',
			url : path + "/pipZtzyInfo/delete?id=" + id,
			success : function(data) {
				if(data=="success"){
